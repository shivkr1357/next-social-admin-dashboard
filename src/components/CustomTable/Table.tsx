...
export default function EnhancedTable<T>({
  ...
  const handleChangeRowsPerPage = (
    ...
    dispatch(paginationActions.setRowPerPage(parseInt(event.target.value, 10)));
  ...
  return (
    <>
                      ...
                      {tableHeadData.map((headCell) => (
                        ...
                        >
                          {headCell.id === "createdAt"
                            ? moment((row as any)["createdAt"]).format(
                                "MMMM Do YYYY"
                              )
                            : headCell.id === "userId"
                            ? (row as any).userId?.fullName
                            : headCell.id === "likes"
                            ? (row as any).likes?.length
                            ...
                            : (row as any)[headCell.id as keyof any]}
                        </TableCell>
                      ))}
                    </TableRow>
                  );
      ...
      <CustomFormModal open={open} handleClose={handleClose} title={title} />
    </>
  );
}
